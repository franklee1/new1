...
unsigned int nTargetSpacing = 10 * 60; // 10 minutes
unsigned int nTargetSpacing_v2 = 5 * 60; // 5 minutes
unsigned int nStakeMinAge = 24 * 60 * 60;
unsigned int nStakeMaxAge = 96 * 60 * 60;
...
const string strMessageMagic = "bigboxcoin Signed Message:\n";
...
int64_t GetProofOfStakeReward(int nHeight, int64_t nCoinAge, int64_t nFees)
{
    int64_t nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8);
            ...
            else if(nBestHeight <= 1000000001)
            {
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "bigboxcoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Big Box Coin 2018";
        CTransaction txNew;
        txNew.nTime = 1525519718;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1525519718;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = !fTestNet ? 946945 : 946945;
        ...
        assert(block.hashMerkleRoot == uint256("0xdede81cc769c3f3c398ada95147b50f5bd16352ab9488f2d6b31c1554fa21e0b"));
